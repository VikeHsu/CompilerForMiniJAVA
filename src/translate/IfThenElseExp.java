...
class IfThenElseExp extends LazyIRTree {
   ...
   public Exp asExp() { 
     ...
     seq = new SEQ(cond.asCond(t, f),
           new SEQ(new LABEL(t.label),                            // T:  
           new SEQ(new MOVE(new TEMP(result.temp), e2.asExp()),   //     result := then expr
                       new LABEL(f.label) )));                        // F:
      } else {
     seq = new SEQ(cond.asCond(t, f),
           new SEQ(new LABEL(t.label),
           new SEQ(new MOVE(new TEMP(result.temp), e2.asExp()),  //      result := then expr
           new SEQ(new JUMP(join.label),                          //      goto join
           new SEQ(new LABEL(f.label),                            // F:
           new SEQ(new MOVE(new TEMP(result.temp), e3.asExp()),  //      result := else expr
